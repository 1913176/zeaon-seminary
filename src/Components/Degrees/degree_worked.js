...
const AddDegreePage = () => {
    ...
    const handleAddCourse = () => {
        setDegreeData({ ...degreeData, courses: [...degreeData.courses, { title: '', description: '', courseThumbnail: null, chapters: [] }] });
    };
    ...
    const handleAddLesson = (courseIndex, chapterIndex) => {
        const updatedCourses = [...degreeData.courses];
        updatedCourses[courseIndex].chapters[chapterIndex].lessons.push({ title: '', file: null });
        setDegreeData({ ...degreeData, courses: updatedCourses });
    };
    ...
    const handleSubmit = async () => {
        ...
        const response = await fetch('https://z-backend-2xag.onrender.com/api/degrees', {
            method: 'POST',
            body: formData
        });
    ...
    return (
        <div>
            <h2>Add Degree</h2>
            <input type="text" placeholder="Title" onChange={(e) => handleDegreeChange('title', e.target.value)} />
            <input type="text" placeholder="Description" onChange={(e) => handleDegreeChange('description', e.target.value)} />
            <input type="number" placeholder="Price" onChange={(e) => handleDegreeChange('price', e.target.value)} />
            <input type="file" onChange={handleFileChange} />
            <button onClick={handleAddCourse}>Add Course</button>
            {degreeData.courses.map((course, courseIndex) => (
                <div key={courseIndex}>
                    <h3>Course {courseIndex + 1}</h3>
                    <input type="text" placeholder="Course Title" onChange={(e) => handleCourseChange(courseIndex, 'title', e.target.value)} />
                    <input type="text" placeholder="Course Description" onChange={(e) => handleCourseChange(courseIndex, 'description', e.target.value)} />
                    <input type="file" onChange={(e) => handleCourseChange(courseIndex, 'courseThumbnail', e.target.files[0])} />
                    <button onClick={() => handleAddChapter(courseIndex)}>Add Chapter</button>
                    {course.chapters.map((chapter, chapterIndex) => (
                        <div key={chapterIndex}>
                            <h4>Chapter {chapterIndex + 1}</h4>
                            <input type="text" placeholder="Chapter Title" onChange={(e) => handleChapterChange(courseIndex, chapterIndex, 'title', e.target.value)} />
                            <button onClick={() => handleAddLesson(courseIndex, chapterIndex)}>Add Lesson</button>
                            {chapter.lessons.map((lesson, lessonIndex) => (
                                <div key={lessonIndex}>
                                    <h5>Lesson {lessonIndex + 1}</h5>
                                    <input type="text" placeholder="Lesson Title" onChange={(e) => handleLessonChange(courseIndex, chapterIndex, lessonIndex, 'title', e.target.value)} />
                                    <input type="file" onChange={(e) => handleLessonChange(courseIndex, chapterIndex, lessonIndex, 'file', e.target.files[0])} />
                                </div>
                            ))}
                        </div>
                    ))}
